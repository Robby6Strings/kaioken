...
export const UseKeyStrokeExample = () => {
  ...
  return (
    <DemoContainer className="p-4 font-cabin h-[350px] flex items-start content-start gap-4 overflow-y-scroll flex-wrap">
      {keysPressed.value.map((key) => {
        return (
          <Button className="text-white rounded-lg capitalize">{key}</Button>
        )
      })}
    </DemoContainer>
  )
