...
export const Group: Kaioken.FC<GroupProps> = (props) => {
  ...
  return (
    <>
      <button
        className="text-[1.125rem] text-left font-semibold"
        onclick={() => (show.value = !show.value)}
      >
        {props.title}
      </button>
      {show.value && (
        <div className="flex flex-col gap-2 border-l-[2px] border-white/20 px-4">
          {props.children}
        </div>
      )}
    </>
...
export const SideBar = () => {
  ...
  return (
    <nav className="w-[300px] hidden md:block self-start sticky top-4 rounded-xl">
      ...
      >
        ...
        <Group title="Browser">
          ...
          <Link href="/browser/useIntersectionObserver">
        ...
        <Group title="Elements">
          ...
          <Link href="/elements/useElementVisibility">
